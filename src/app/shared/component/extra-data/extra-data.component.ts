import {
  ...
  extraDataShowHideColAnimation,
  extraDataShowHideRowAnimation,
  extraDataShowHideEChartAnimation,
} from 'src/animation';
import {
...
export class ExtraDataComponent implements OnInit, OnDestroy, OnChanges {
  ...
  readonly productPropList: (keyof ProductResult)[] = ['product'];
  ...
  //属性情報所属フラグ
  @Input('isFromInfo') isFromInfo: boolean = false;
      ...
      formatter: (param: any) => {
        ...
        param.forEach((v: any) => {
          ...
          if (!isOtherProp) {
            ...
            const addVal =
              this.genshinDataService.getOptimalReliquaryAffixStep(prop) * 10 * parseInt(v.name);
            let sign = '';
            let valStr = '';
            if (v.name != 0) {
              if (
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (
      changes['valueIndexes'] &&
      (changes['valueIndexes'].previousValue?.length !==
        changes['valueIndexes'].currentValue?.length ||
        changes['valueIndexes'].previousValue?.some(
          (element: any, index: number) => element != changes['valueIndexes'].currentValue[index],
        ))
    ) {
      this.initDamageDatas();
      this.calEchartsDatas(type_damage);
  ...
  onChangeSwitch(change: MatSlideToggleChange, valueIndex: number) {
    this.calculatorService.setSkillBuffValue(
      this.characterIndex,
      this.skill,
      valueIndex,
      'switch',
  ...
  async calEchartsDatas(type: ValueType, reCalc: boolean = true) {
    ...
    (this as any)[currentClacProcessOptionProp] = this.deepClone(
      this.commonCalcProcessEchartsOption,
    );
    //ーーーーーーーーーーーーーー
    //ーーーーダメージ計算ーーーー
    //ーーーーーーーーーーーーーー
    ...
    //現在武器精錬レベル
    const currentSmeltingLevel = parseInt(
    ...
    awaitArray.push(
      lastValueFrom(
        this.translateService.get(`GENSHIN.${valueI18nPrefix}.${this.propNameMap[currentProp]}`),
      ),
    );
  ...
  updateRelayout(msg = 'animation') {
    setTimeout(() => {
      this.relayoutMsgService.update(msg);
    }, 10);
  }

  private getDmgInfos() {
    let temp = this.calculatorService.getSkillDmgValue(
      this.characterIndex,
      this.skill,
      this.valueIndexes,
  ...
  private getHealingInfos() {
    let temp = this.calculatorService.getSkillHealingValue(
      this.characterIndex,
      this.skill,
      this.valueIndexes,
      this.skillIndex,
  ...
  private getShieldInfos() {
    let temp = this.calculatorService.getSkillShieldValue(
      this.characterIndex,
      this.skill,
      this.valueIndexes,
      this.skillIndex,
  ...
  private getProductInfos() {
    let temp = this.calculatorService.getSkillProductHpValue(
      this.characterIndex,
      this.skill,
      this.valueIndexes,
      this.skillIndex,
  ...
  private getBuffInfos() {
    let temp = this.calculatorService.getSkillBuffValue(
      this.characterIndex,
      this.skill,
      this.skillIndex,
      this.valueIndexes,
